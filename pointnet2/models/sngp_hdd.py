...
import os, sys
# add project root so absolute import also works if needed
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # .../pointnet2
...
def chunked_knn_indices(xyz_bnc, k, chunk_size=256):
    """
    xyz_bnc: (B,N,3), return idx: (B,N,k).
    Computes KNN without building full NxN distance matrix.
    """
    B, N, _ = xyz_bnc.shape
    ...
    for b in range(B):
        ...
        for start in range(0, N, chunk_size):
            ...
            # dist^2 = ||q||^2 + ||x||^2 - 2 q x^T
            QX = Q @ X.t()                           # (C,N)
            dist2 = Q_sq + X_sq.unsqueeze(0) - 2.0 * QX  # (C,N)

            # exclude self for rows that align to [start:end)
            row_idx = torch.arange(start, end, device=device)
            col_idx = row_idx  # same index means self
            dist2[torch.arange(end-start, device=device), col_idx] = float('inf')

            topk = torch.topk(dist2, k=k, largest=False, dim=1).indices  # (C,k)
...
def gather_neighbors(x, idx):
    ...
    neigh = torch.take_along_dim(x_exp, idx_exp, dim=2)
    return neigh
...
class get_model(nn.Module):
    ...
    def forward(self, xyz, cls_label=None, return_point_feats=False, compute_sigma=True):
        B, _, N = xyz.shape
        x_bnc = xyz.transpose(1, 2).contiguous()

        h_bnc = self.geom_tf(x_bnc)                # (B,N,Ct)
        _, sigma2_geo = self.early_rffgp(
            h_bnc, update_precision=self.training, compute_var=compute_sigma
        )
        ...
        l0_points = self.fp0(l0_xyz, l1_xyz, torch.cat([l0_xyz, l0_points_raw], dim=1), l1_points)
