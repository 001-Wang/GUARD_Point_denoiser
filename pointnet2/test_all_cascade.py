...
def _robust_minmax(x: torch.Tensor, lo: float, hi: float) -> torch.Tensor:
    # clamp NaN/Inf and quantile-range normalize to ~[0,1]
    x = torch.nan_to_num(x, posinf=1e6, neginf=-1e6)
    ql = torch.quantile(x, lo, dim=1, keepdim=True)
    qh = torch.quantile(x, hi, dim=1, keepdim=True)
    den = torch.clamp(qh - ql, min=1e-6)
    z = (x - ql) / den
...
def extract_uncertainty(
    ...
    valid_parts_bc: Optional[torch.Tensor] = None,   # 若评测启用类别mask，传入
    mode: str = "cascade",                           # "sigma" | "entropy" | "cascade"
    r: float = 0.6,                                  # 级联：第一步SNGP比例
    K: Optional[int] = None,                         # 级联：目标保留数（必须提供）
    q_lo: float = 0.10, q_hi: float = 0.90,          
):
    assert mode in {"sigma", "entropy", "cascade"}, f"Invalid mode={mode}"
    assert 0.0 < q_lo < q_hi < 1.0

    B, N, C = logits_bnc.shape
    device = logits_bnc.device
    eps = 1e-6


    def _get_sigma2(aux_dict: Dict[str, torch.Tensor]) -> torch.Tensor:
        keys = ['sigma2_geo','sigma2_sem','sigma2','sigma','var','variance','log_var','logvar']
        for k in keys:
            v = aux_dict.get(k, None)
            if v is None:
                continue
            t = v
            if t.dim() == 3 and t.shape[:2] == (B, N):  # (B,N,*) -> mean over last
                t = t.mean(dim=-1)
            ...
            if 'log' in k:
                t = t.exp()
            return t
        raise RuntimeError("[SNGP] sigma-like key not found in aux.")

    # ---- compute normalized entropy / sigma2 ----
    logits_eff = _masked_logits(logits_bnc, valid_parts_bc)
    ent_bn = _entropy_from_logits(logits_eff)             
    h_norm = _robust_minmax(ent_bn, q_lo, q_hi)                        

    if mode == "entropy":
        return h_norm + eps * h_norm                       

    s2_bn = _get_sigma2(aux)                               
    s_norm = _robust_minmax(s2_bn, q_lo, q_hi)                        

    if mode == "sigma":
        return s_norm + eps * h_norm                       

    # ---- cascade ----
    assert K is not None and K > 0, "When mode='cascade', K must be provided."
    kept_idx_list = []
    K_eff = min(int(K), N)
    K1 = min(int(round(r * K_eff)), K_eff)
    K2 = K_eff - K1

    for b in range(B):
        idx1 = torch.argsort(s_norm[b], dim=0, stable=True)[:K1]

        mask = torch.ones(N, dtype=torch.bool, device=device)
        mask[idx1] = False
        ent_rem = h_norm[b].masked_fill(~mask, float('inf'))
        idx2 = torch.argsort(ent_rem, dim=0, stable=True)[:K2]

        kept_idx = torch.sort(torch.cat([idx1, idx2], dim=0))[0]    # (K_eff,)
        kept_idx_list.append(kept_idx)

    return torch.stack(kept_idx_list, dim=0)   # (B, K_eff)
...
def main():
    ...
    parser.add_argument('--model_cvar', type=str, default='pointnet2.models.pointnet2_part_seg_msg')
    parser.add_argument('--ckpt_cvar', type=str, default='pointnet2/log/without_normal/checkpoints/best_model.pth')
    ...
    parser.add_argument('--model_entropy', type=str, default='pointnet2.models.sngp_s2_6layers')
    parser.add_argument('--ckpt_entropy', type=str, default='pointnet2/log/sngp/checkpoints/best_model.ckpt')
    parser.add_argument('--model_sngp', type=str, default='pointnet2.models.sngp_s2_6layers')
    parser.add_argument('--ckpt_sngp', type=str, default='pointnet2/log/sngp/checkpoints/best_model.ckpt')
    parser.add_argument('--precision_path', type=str, default='pointnet2/log/sngp/checkpoints/p_clean.pt')
    parser.add_argument('--unc_mode', type=str, default='hybrid',
                    choices=['cascade', 'entropy', 'sigma'],
                    help='Uncertainty extraction mode: hybrid | entropy | sigma')
    ...
    if args.method == 'sngp':
        if not args.precision_path:
            raise RuntimeError("--precision_path is required for SNGP")
        P = torch.load(args.precision_path, map_location=device).float()
        P = 0.5 * (P + P.T)  # symmetrize
        # (optional but recommended) small ridge for stability:
        ridge = 10
        P = P + ridge * torch.eye(P.shape[0], device=P.device)
    ...
    for it, batch in enumerate(tqdm(loader, total=len(dataset), desc=f"{args.method} eval", ncols=80)):
        ...
        if args.method == 'pointcvar':
            ...
            # --- configs & basics ---
            valid_parts_bc = build_valid_parts(label_B) if args.mask_allowed else None
            unc_mode   = getattr(args, 'unc_mode', 'cascade')   # "cascade" | "sigma" | "entropy"
            K_target   = int(getattr(args, 'keep', 2048))
            r          = float(getattr(args, 'cascade_r', 1)) # only used when cascade
            q_lo       = float(getattr(args, 'q_lo', 0.10))
            q_hi       = float(getattr(args, 'q_hi', 0.90))
            N          = logits_full_bnc.size(1)                # keep your N for print
            K          = min(K_target, N)                       # keep your K for print & consistency

            # --- selection ---
            if unc_mode == 'cascade':
                # returns (B, K_eff), assume B==1 downstream like your original code
                kept_idx_bk = extract_uncertainty(
                    aux,
                    logits_full_bnc,
                ...
                idx_sorted = kept_idx_bk[0]                     # Tensor of size (K_eff,)
                K = int(idx_sorted.numel())                     # keep printed K in sync

            else:
                # single-score: "sigma" or "entropy" → low is good
                unc_bn = extract_uncertainty(
                    aux,
                    logits_full_bnc,
                    valid_parts_bc=valid_parts_bc,
                    mode=unc_mode,                               # "sigma" or "entropy"
                    q_lo=q_lo, q_hi=q_hi,
                )
                K = min(K, unc_bn.shape[1])                     # re-affirm with actual N
                idx_sorted = torch.argsort(unc_bn[0], dim=0, stable=True)[:K]

            kept_counts.append(int(K))

            # --- bookkeeping on noise ---
            noise_kept = int((gt_n[idx_sorted] == -1).sum().item())
            remain_noise_counts.append(noise_kept)
    ...
    print("[SUMMARY]")
    print(f"Samples evaluated: 900-1900")
