...
def parse_args():
    ...
    parser.add_argument('--log_dir', type=str, default='pointnet2\log\without_normal_hdd', help='experiment root')
    parser.add_argument('--normal', action='store_true', default=False, help='use normals')
    parser.add_argument('--num_votes', type=int, default=3, help='aggregate segmentation scores with voting')
    parser.add_argument('--root', type=str, default='data_prepare\hdd_data', help='dataset root')
...
# --- BEGIN PATCH: robust eval that penalizes -1 and avoids KeyError ---
    with torch.no_grad():
        ...
        for batch_id, (points, label, target) in tqdm(enumerate(testDataLoader),
            ...
            cur_pred_val_logits = seg_pred.detach().cpu().numpy()  # [B, N, 5]
