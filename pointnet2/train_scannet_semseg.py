...
from data_utils.ScanNetDataLoader import ScanNetPTHSceneDataset
...
def build_class_weights(dataset, num_classes, ignore_index=255, max_scenes_scan=9999, clamp=(0.2, 5.0)):
    ...
    # inverse sqrt is commonly more stable than inverse
    w = 1.0 / torch.sqrt(freq.clamp_min(1e-6))
    w = w / w.mean().clamp_min(1e-6)

    lo, hi = clamp
    w = torch.clamp(w, lo, hi)
    return w
...
def main():
    ...
    # dataset
    train_set = ScanNetPTHSceneDataset(
        root=args.data_root, split="train",
        npoints=args.npoint, use_rgb=args.use_rgb,
        normalize_xyz=args.normalize_xyz, repeat=args.repeat, seed=0
    )
    val_set = ScanNetPTHSceneDataset(
        root=args.data_root, split="val",
        npoints=args.npoint, use_rgb=args.use_rgb,
        normalize_xyz=args.normalize_xyz, repeat=max(1, args.repeat // 5), seed=1
    )
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for _, (pts, y, scene_name) in tqdm(enumerate(train_loader), total=len(train_loader), smoothing=0.9):
            ...
            if pts.dim() == 4:
                B, K, N, C = pts.shape
                pts = pts.view(B*K, N, C)
                y   = y.view(B*K, N)

            if epoch == 0 and _ == 0:
                y_flat0 = y.reshape(-1)
                valid0 = y_flat0[y_flat0 != args.ignore_index]
                print("scene_name:", scene_name[:2] if isinstance(scene_name, (list, tuple)) else scene_name)
                print("y shape:", tuple(y.shape))
                print("valid count:", valid0.numel(), "/", y_flat0.numel())
                if valid0.numel() > 0:
                    print("valid labels min/max:", valid0.min().item(), valid0.max().item())
                    uniq = torch.unique(valid0)
                    print("valid labels unique head:", uniq[:30].tolist(), " (#uniq=", int(uniq.numel()), ")")
                print("ignore count:", int((y_flat0 == args.ignore_index).sum().item()))
            ...
            with autocast(enabled=args.amp):
                ...
                if sngp_logits is None:
                    ...
                    w = class_w.to(dtype=logits_flat.dtype)
                    loss = F.cross_entropy(logits_flat, y_flat, weight=w)
        ...
        with torch.no_grad():
            for _, (pts, y, scene_name) in tqdm(enumerate(val_loader), total=len(val_loader), smoothing=0.9):
                ...
                if sngp_logits is None:
                    ...
                    w = class_w.to(dtype=logits_flat_v.dtype)
                    loss = F.cross_entropy(logits_flat_v, y_flat_v, weight=w)
